...
read_file(Path, Options) ->
  {ok, TS} = file:read_file(Path),
  {ok, Dec0} = init([Options]),
  decode(TS, Dec0).
...
decode(Bin, #decoder{frames = Buffered} = Decoder, Frames) ->
  Sorted = lists:sort(fun frame_sorter/2, Buffered ++ Frames),
...
decode_ts(<<_Error:1, PayloadStart:1, _TransportPriority:1, Pid:13, _Scrambling:2,
            _HasAdaptation:1, _HasPayload:1, _Counter:4, _TSRest/binary>> = Packet, #decoder{pids = Pids, dump_psi = DumpPSI} = Decoder) ->
  PCR = get_pcr(Packet),
